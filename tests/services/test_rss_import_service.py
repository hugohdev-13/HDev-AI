...
def test_new_rss_entry_calls_ai_once(_source, feed, duplicate, create, _save, _categories):
...
@patch("services.rss_import_service.CategoryService.get_active_categories", return_value=[])
@patch("services.rss_import_service.SourceRepository.save")
def test_duplicate_does_not_consume_ai(_save, _categories, _duplicate, _source, feed):
    feed.return_value = RSSFeedResult(True, [_entry()], None, None)
